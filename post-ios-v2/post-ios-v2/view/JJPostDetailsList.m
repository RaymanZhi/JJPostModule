...
#import "JJPostListItem.h"

@interface JJPostDetailsList()

@property (nonatomic,strong) UIView *sectionHeaderView;

@property (nonatomic,strong) NSMutableArray *allItems;

@property (nonatomic,strong) JJPostListItem *itemSelect;

@end

@implementation JJPostDetailsList
...
#pragma 更多频道标签
    ...
    for (int i =0; i <count1; i++) {
        JJPostListItem *item = [[JJPostListItem alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i% itemPerLine), padding+(kItemH + padding)*(i/itemPerLine), kItemW, kItemH)];
        item.longPressBlock = ^(){
        ...
        if (!self.itemSelect) {
            [item setTitleColor:[UIColor blueColor] forState:0];
    ...
    for (int i=0; i<count2; i++) {
        JJPostListItem *item = [[JJPostListItem alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine),CGRectGetMaxY(self.sectionHeaderView.frame)+padding+(kItemH+padding)*(i/itemPerLine), kItemW, kItemH)];
        item.operationBlock = ^(animateType type, NSString *itemName, int index){
            if (self.opertionFromItemBlock) {
                self.opertionFromItemBlock(type,itemName,index);
            }
...
-(void)itemRespondFromListBarClickWithItemName:(NSString *)itemName{
    for (int i = 0 ; i<self.allItems.count; i++) {
        JJPostListItem *item = (JJPostListItem *)self.allItems[i];
