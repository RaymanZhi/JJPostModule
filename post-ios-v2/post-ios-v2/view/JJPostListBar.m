...
@implementation JJPostListBar

- (void)setVisibleItemList:(NSMutableArray *)visibleItemList
{
    _visibleItemList = visibleItemList;
    if (!self.btnBackView) {
        self.btnBackView = [[UIView alloc] initWithFrame:CGRectMake(10, (self.frame.size.height - 20)/2 + 20, 46, 2)];
        self.btnBackView.backgroundColor = RGBColor(54, 179, 249);
...
- (void)makeItemWithTitle:(NSString *)title
{
    ...
    [item setTitleColor:RGBColor(111, 111, 111) forState:UIControlStateNormal];
    [item setTitleColor:RGBColor(111, 111, 111) forState:UIControlStateHighlighted];
    [item setTitleColor:RGBColor(54, 179, 249) forState:UIControlStateSelected];
    ...
    if (!self.btnSelect) {
        [item setTitleColor:RGBColor(54, 179, 249) forState:0];
        self.btnSelect = item;
    }
    self.contentSize = CGSizeMake(self.maxWidth, self.frame.size.height);
}
...
- (void)itemClick:(UIButton *)sender
{
    if (self.btnSelect != sender) {
        ...
        [sender setTitleColor:RGBColor(54, 179, 249) forState:0];
